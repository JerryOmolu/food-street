...
function sendemail_verify($email, $phone, $verify_token){
    ...
    try {
        ...
        $mail->Password   = 'michaelking123';        // ⚠️ Best practice: move this to .env/config
        ...
        $verify_link = "http://localhost/food-street/verify_email_agent.php?token=$verify_token&email=$email&phone=$phone";
...
if (isset($_POST['register_agent'])) {
    ...
    if (empty($email)) {
        ...
        $stmt = $connection->prepare("SELECT email FROM agent WHERE email = ? LIMIT 1");
    ...
    if (empty($phone)) {
        ...
        $stmt = $connection->prepare("SELECT phone FROM agent WHERE phone = ? LIMIT 1");
    ...
    if (empty($errors)) {
        ...
        $stmt = $connection->prepare("INSERT INTO agent 
            (full_name, email, phone, password, role, added_on, verify_token, reg_amount, reg_status, added_by) 
            VALUES (?, ?, ?, ?, ?, NOW(), ?, ?, ?, ?)");
        ...
        if ($stmt->execute()) {
            ...
            $_SESSION['status'] = "Registration as Agent is successful. Please check '$email' for verification link.";
